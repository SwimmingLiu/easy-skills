...
TASK="$1"
WORKDIR="${2:-$(pwd)}"
SESSION_NAME="${3:-opencode-$(date +%Y%m%d-%H%M%S)}"
...
set -euo pipefail
cd "$WORKDIR"
printf 'Launching OpenCode TUI in %s\n' "$WORKDIR"
printf 'Prompt file: %s\n' "$PROMPT_FILE"
printf 'The task text has been copied to the tmux paste buffer.\n'
printf 'Paste it into OpenCode with Ctrl-Shift-V, Shift-Insert, or tmux paste-buffer.\n\n'
exec opencode 2>&1 | tee -a "$LOG_FILE"
EOF
