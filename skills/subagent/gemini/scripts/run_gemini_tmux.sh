...
TASK="$1"
WORKDIR="${2:-$(pwd)}"
SESSION_NAME="${3:-gemini-$(date +%Y%m%d-%H%M%S)}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SKILL_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
OUTPUT_DIR="$SKILL_DIR/.tmp/tmux"

command -v gemini >/dev/null 2>&1 || fail "$EXIT_GEMINI_MISSING" "gemini not found in PATH"
...
set -euo pipefail
cd "$WORKDIR"
printf 'Launching Gemini TUI in %s\n' "$WORKDIR"
printf 'Prompt file: %s\n' "$PROMPT_FILE"
printf 'The task text has been copied to the tmux paste buffer.\n'
printf 'Paste it into Gemini with Ctrl-Shift-V, Shift-Insert, or tmux paste-buffer.\n\n'
exec gemini 2>&1 | tee -a "$LOG_FILE"
EOF
