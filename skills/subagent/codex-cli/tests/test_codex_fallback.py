...
# ///
"""
Codex Skill 降级功能测试脚本

测试场景：
1. 模拟 codex CLI 成功输出（JSON agent_message）
2. 模拟 codex CLI 失败（各种错误场景）
3. 验证自动降级到 OpenCode review agent
4. 测试输出文件保存功能
5. 测试 resume 模式（模拟）
"""
...
def create_mock_codex_cli(success: bool = True, error_type: str = "rate_limit") -> str:
    """
    创建模拟的 codex CLI 脚本
    ...
    mock_script = Path(temp_dir) / "codex"

    if success:
        # 模拟成功的 codex CLI JSON 输出
        thread_id = "test-thread-12345"
        agent_message = "这是 Codex CLI 的模拟输出。发现了几个性能问题..."

        content = f"""#!/bin/bash
echo '{{"type":"thread.started","thread_id":"{thread_id}"}}'
echo '{{"type":"item.completed","item":{{"type":"agent_message","text":"{agent_message}"}}}}'
exit 0
"""
    else:
        # 模拟失败的 codex CLI
...
def run_test_with_mock_cli(
    mock_cli_path: str, test_name: str, mode: str = "new"
) -> Tuple[bool, str, str, int]:
    """
    使用模拟的 CLI 运行测试

    Args:
        mock_cli_path: 模拟 CLI 路径
        test_name: 测试名称
        mode: "new" 或 "resume"
    ...
    try:
        if mode == "resume":
            result = subprocess.run(
                ["python3", CODEX_SCRIPT, "resume", "test-session-id", TEST_TASK],
            ...
            result = subprocess.run(
                ["python3", CODEX_SCRIPT, TEST_TASK],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
                timeout=30,
...
def test_normal_success():
    """测试 1: 正常调用成功"""
    print_test_header("测试 1: Codex CLI 正常调用")

    # 创建模拟成功的 codex CLI
    mock_cli = create_mock_codex_cli(success=True)
    ...
    try:
        ...
        if success:
            ...
            # 检查输出中是否包含 SESSION_ID
            if "SESSION_ID" in stdout:
                print_success("包含 SESSION_ID（符合预期）")
            else:
                print_warning("未发现 SESSION_ID")

            return True
        else:
            print_error(f"Codex CLI 调用失败: {stderr}")
            return False
...
def test_fallback_scenarios():
    ...
    for i, (error_type, description) in enumerate(scenarios, start=2):
        ...
        # 创建模拟失败的 codex CLI
        mock_cli = create_mock_codex_cli(success=False, error_type=error_type)
        ...
        try:
            ...
            opencode_used = "opencode" in stderr.lower() or "review" in stderr.lower()

            if fallback_triggered:
                print_success("✓ 成功触发降级机制")
                print_info(f"错误信息: {stderr[:200]}...")

                if opencode_used:
                    print_success("✓ 正确调用 OpenCode review agent")
                else:
                    print_warning("⚠ 未明确提示使用 OpenCode review agent")

                results.append(True)
            else:
                print_error("✗ 未能触发降级机制")
                print_info(f"返回码: {returncode}")
...
def test_resume_mode():
    """测试 8: resume 模式"""
    print_test_header("测试 8: resume 模式")
    ...
    try:
        success, stdout, stderr, returncode = run_test_with_mock_cli(
            mock_cli, "resume_mode", mode="resume"
        )

        if success:
            print_success("resume 模式调用成功")
            print_info(f"输出: {stdout[:100]}...")
            return True
        else:
            print_error(f"resume 模式调用失败: {stderr}")
            return False
...
def test_fallback_script_directly():
    """测试 9: 直接调用降级脚本"""
    print_test_header("测试 9: 直接调用 codex_fallback.py")
    ...
    try:
        ...
        # 直接调用降级脚本
        result = subprocess.run(
            ["python3", CODEX_FALLBACK_SCRIPT, "测试task: 分析代码质量"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
            timeout=60,
...
def test_output_file_saving():
    """测试 10: 验证输出文件保存功能"""
    print_test_header("测试 10: 输出文件保存")

    output_dir = Path(".tmp/docs/output")

    # 记录测试前的文件数量
    if output_dir.exists():
        files_before = list(output_dir.glob("out_*_codex*.md"))
    ...
    try:
        ...
        # 检查是否生成了新文件
        if output_dir.exists():
            files_after = list(output_dir.glob("out_*_codex*.md"))
...
def run_all_tests():
    """运行所有测试"""
    print(f"\n{Colors.BLUE}{'=' * 60}{Colors.RESET}")
    print(f"{Colors.BLUE}Codex Skill 降级功能测试套件{Colors.RESET}")
    print(f"{Colors.BLUE}{'=' * 60}{Colors.RESET}")
    ...
    # 测试 8: resume 模式
    results["resume 模式"] = test_resume_mode()
