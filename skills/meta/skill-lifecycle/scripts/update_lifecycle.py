...
def reject_symlink_path(path):
    if Path(path).is_symlink():
        raise LifecycleError(f"refusing symlink lifecycle path: {path}")


def command_init(arguments):
    reject_symlink_path(arguments.file)
    with lifecycle_lock(arguments.file):
        reject_symlink_path(arguments.file)
        command_init_locked(arguments)


def command_init_locked(arguments):
...
def command_transition(arguments):
    ...
    with lifecycle_lock(arguments.file):
        reject_symlink_path(arguments.file)
        command_transition_locked(arguments)


def command_transition_locked(arguments):
...
def command_rollback(arguments):
    ...
    with lifecycle_lock(arguments.file):
        reject_symlink_path(arguments.file)
        command_rollback_locked(arguments)


def command_rollback_locked(arguments):
    document = load_document(arguments.file)
