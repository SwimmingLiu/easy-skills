...
Before replacement, write, flush, file-fsync, and replace failures raise an error, preserve the prior destination byte-for-byte, and clean the sibling temporary file. After replacement on POSIX, the containing directory is synchronized so the directory entry is crash-durable. `atomic_write_json` returns true when this durability step succeeds. If only the post-replace directory fsync fails, replacement is already committed and the function returns false rather than reporting an uncommitted failure. Mutating CLI commands then exit successfully with a stderr warning that the data is committed but directory durability could not be confirmed and that current state must be inspected before any retry.
