...
test('help documents the mutually exclusive image-first and pure HTML modes', () => {
  ...
  assert.match(result.stdout, /pure-html/);
  assert.doesNotMatch(result.stdout, /html-image-assisted/);
...
test('pure HTML mode skips image generation and routes directly to HTML rendering', async () => {
  const parent = await mkdtemp(join(tmpdir(), 'image-ppt-cli-'));
  const root = join(parent, 'deck');
  assert.equal(run(['draft', root], parent).status, 0);
  assert.equal(run(['approve', root, '--theme', 'editorial', '--mode', 'pure-html'], parent).status, 0);
  const deck = JSON.parse(await readFile(join(root, 'deck_spec.json'), 'utf8'));
  ...
  const jobs = (await readFile(join(root, 'imagegen-jobs.jsonl'), 'utf8')).trim();
  assert.equal(jobs, '');
  assert.equal(run(['render', root], parent).status, 0);
  const rendered = await readFile(join(root, 'index.html'), 'utf8');
  assert.match(rendered, /data-mode="pure-html"/);
  assert.doesNotMatch(rendered, /<img\b|data:image|html-image-assisted/i);
});
...
test('pure HTML bundle contains exact copy without image assets', async () => {
  const parent = await mkdtemp(join(tmpdir(), 'image-ppt-cli-'));
  const root = join(parent, 'deck');
  assert.equal(run(['draft', root], parent).status, 0);
  assert.equal(run(['approve', root, '--theme', 'editorial', '--mode', 'pure-html'], parent).status, 0);
  ...
  assert.match(rendered, /pure-html/);
  assert.doesNotMatch(rendered, /<img\b|data:image/i);
  assert.equal(run(['bundle', root], parent).status, 0);
  const html = await readFile(join(root, 'dist', 'presentation.pure-html.html'), 'utf8');
  assert.match(html, /application\/image-ppt\+json/);
  assert.match(html, /AI 原生知识工作流/);
  assert.match(html, /"mode":"pure-html"/);
  assert.doesNotMatch(html, /data:image|<img\b|html-image-assisted/i);
});

test('pure HTML export report preserves the selected mode', async () => {
  const parent = await mkdtemp(join(tmpdir(), 'image-ppt-cli-'));
  const root = join(parent, 'deck');
  assert.equal(run(['draft', root], parent).status, 0);
  assert.equal(run(['approve', root, '--theme', 'editorial', '--mode', 'pure-html'], parent).status, 0);
  ...
  assert.equal(report.output_mode, 'pure-html');
});
