...
核心运行时不绑定 ChatGPT、Chat Image、gpt-image-2、Image Two 或其他供应商。调用当前宿主可用的图片生成能力，并以 manifest 保存实际 prompt、模型和输出文件。当前 CLI 任务由 `prompts` 写入 `imagegen-jobs.jsonl`，生图完成后将 PNG 放入对应的 `slides/` 路径。

整页图片必须 `fact_bearing: false`，除非文字和数字来自用户确认的 exact text 或受控素材。避免图内长文字、Logo、真实数据、真实产品截图和论文结果。所有需要呈现的文字必须先进入内容草稿并原样传给模型；不再依赖 HTML 文字覆盖层。
