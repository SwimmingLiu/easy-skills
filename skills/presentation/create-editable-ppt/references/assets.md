...
核心运行时不绑定 ChatGPT、Chat Image、gpt-image-2、Image Two 或其他供应商。只有 `image-first` 调用当前宿主可用的图片生成能力，并以 manifest 保存实际 prompt、模型和输出文件；其 PNG 放入 `slides/`。`pure-html` 不调用生图、不生成 `assets/`，所有视觉结构由 HTML/CSS 完成。

整页图片必须 `fact_bearing: false`，除非文字和数字来自用户确认的 exact text 或受控素材。避免图内长文字、Logo、真实数据、真实产品截图和论文结果。`image-first` 的文字必须先进入内容草稿并原样传给模型；`pure-html` 的文字由 HTML 从内容草稿读取，不需要图片提示词。
