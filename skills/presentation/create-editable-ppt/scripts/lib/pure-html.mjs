...
function artMarkup() {
  return '<div class="art" aria-hidden="true"><span class="signal signal-a"></span><span class="signal signal-b"></span><span class="signal signal-c"></span></div>';
}

function slideMarkup(source, index, total) {
  const role = source.role ?? 'statement';
  const fullBleed = ['cover', 'closing', 'image-hero'].includes(role);
  return `<section class="slide mode-pure-html role-${escapeHtml(role)}${fullBleed ? ' full-bleed' : ''}${index === 0 ? ' active' : ''}" data-slide="${index}" data-slide-id="${escapeHtml(source.id)}" data-mode="pure-html">${artMarkup()}${copyMarkup(source)}<span class="counter">${index + 1} / ${total}</span></section>`;
}

function styles(tokens) {
  return `*{box-sizing:border-box}html,body{margin:0;background:${tokens.background};color:${tokens.text};font-family:${tokens.body};letter-spacing:0}body{min-height:100vh}.deck{height:100vh;display:grid;place-items:center;padding:24px}.slide{display:none;position:relative;overflow:hidden;width:min(calc(100vw - 48px),calc(177.78vh - 48px));aspect-ratio:16/9;background:${tokens.surface};border:1px solid ${tokens.line};border-radius:${tokens.radius};box-shadow:0 18px 50px #0002}.slide.active{display:grid}.art{position:absolute;inset:10% 6% 10% 57%;overflow:hidden;background:${tokens.background};border:1px solid ${tokens.line};border-radius:${tokens.radius}}.art:before{content:"";position:absolute;width:46%;aspect-ratio:1;border:1px solid ${tokens.accent};border-radius:50%;right:10%;top:17%;opacity:.72}.art:after{content:"";position:absolute;width:70%;height:1px;background:${tokens.support};right:-5%;top:65%;transform:rotate(-25deg);transform-origin:right center;opacity:.82}.signal{position:absolute;display:block;background:${tokens.accent};opacity:.86}.signal-a{width:18%;height:18%;left:12%;bottom:14%}.signal-b{width:30%;height:1px;left:22%;bottom:28%;transform:rotate(90deg);transform-origin:left center;background:${tokens.support}}.signal-c{width:10%;height:10%;right:22%;bottom:16%;border:1px solid ${tokens.accent};background:transparent}.copy{position:relative;z-index:1;align-self:center;width:56%;padding:7%;max-width:760px}.copy h1{margin:0 0 24px;font-family:${tokens.heading};font-size:clamp(28px,4.2vw,72px);line-height:1.08;letter-spacing:0}.copy p{margin:12px 0;color:${tokens.muted};font-size:clamp(16px,1.5vw,28px);line-height:1.45}.steps{display:grid;gap:12px;list-style:none;padding:0;margin:26px 0 0;counter-reset:step}.steps li{counter-increment:step;display:flex;align-items:center;gap:12px;color:${tokens.text};font-size:clamp(16px,1.45vw,25px);line-height:1.2}.steps li:before{content:counter(step,decimal-leading-zero);display:grid;place-items:center;flex:none;width:42px;height:32px;border:1px solid ${tokens.accent};border-radius:${tokens.radius};color:${tokens.accent};font-size:12px}.full-bleed{display:block;background:${tokens.text}}.full-bleed .art{inset:0;border:0;border-radius:0;background:${tokens.text}}.full-bleed .art:before{width:48%;right:9%;top:23%;border-color:${tokens.accent};opacity:.75}.full-bleed .art:after{background:${tokens.support};top:60%;opacity:.75}.full-bleed .signal-a{background:${tokens.accent}}.full-bleed .signal-b{background:${tokens.support}}.full-bleed .signal-c{border-color:${tokens.accent}}.full-bleed .copy{position:absolute;left:0;top:0;height:100%;display:flex;justify-content:center;flex-direction:column;color:${tokens.surface};width:63%}.full-bleed .copy p{color:${tokens.surface}cc}.counter{position:absolute;z-index:2;right:18px;bottom:14px;padding:5px 8px;background:${tokens.text}aa;color:${tokens.surface};font:12px system-ui,sans-serif}.nav{position:fixed;top:50%;width:42px;height:42px;transform:translateY(-50%);border:0;border-radius:50%;background:${tokens.text}dd;color:${tokens.surface};font-size:24px;cursor:pointer}.nav:focus-visible{outline:2px solid ${tokens.accent};outline-offset:3px}#prev{left:10px}#next{right:10px}@media(max-width:760px){.deck{padding:0}.slide{width:100vw;border:0;border-radius:0}.art{inset:48% 6% 8%}.copy{width:100%;padding:8%;align-self:start}.copy h1{font-size:clamp(28px,8vw,48px)}.full-bleed .copy{width:92%;padding:8%}.full-bleed .art{inset:0}.role-process .art,.role-comparison .art,.role-data .art,.role-two-column .art{inset:52% 6% 8%}.role-process .copy,.role-comparison .copy,.role-data .copy,.role-two-column .copy{padding-top:8%}}@media print{body{background:${tokens.surface}}.deck{height:auto;display:block;padding:0}.slide{display:block;width:100vw;height:56.25vw;border:0;border-radius:0;box-shadow:none;page-break-after:always}.nav,.counter{display:none}}@media screen{.slide.full-bleed{display:none}.slide.active.full-bleed{display:block}}.full-bleed .copy h1{font-size:clamp(26px,3.8vw,64px);white-space:nowrap}`;
}
