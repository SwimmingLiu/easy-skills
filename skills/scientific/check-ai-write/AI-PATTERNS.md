...
本文件包含 24 种 AI 写作模式的完整列表，每种模式包含需要注意的词汇和改写前后对比示例。基于 [Wikipedia:Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing)。
...
**注：** 中文标题通常不涉及大小写问题，此模式在中文中不太适用。在英文文本中注意将 Title Case 改为 Sentence case。
...
**注：** ChatGPT 使用弯引号（\u201c\u201d）而不是直引号（""）。中文通常使用中文引号（「」或\u201c\u201d），此模式在中文中表现为英文引号的使用。
