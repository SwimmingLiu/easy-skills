...
def choose_engine(engine: str) -> Literal["mlx", "faster-whisper"]:
    ...
    if is_apple_silicon():
        try:
            ...
            raise SystemExit(
                "Apple Silicon detected, but MLX GPU transcription is not ready. "
                "Install or repair mlx-whisper in a virtual environment and rerun "
                "with --engine mlx. Do not fall back to CPU unless the user "
                f"explicitly approves it. Original error: {type(exc).__name__}: {exc}"
            ) from exc

        raise SystemExit(
            "Apple Silicon detected, but MLX is not using Device(gpu, 0). "
            "Repair MLX/Metal and rerun with --engine mlx. Do not fall back to CPU "
