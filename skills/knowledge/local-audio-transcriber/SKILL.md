...
3. Apple Silicon（M1/M2/M3/M4）必须安装并使用 MLX，本地调用 Apple GPU/统一内存。除非用户明确同意，不要降级到 CPU：
...
4. 非 Apple Silicon、CUDA 机器，或用户明确同意 CPU fallback 时，才使用 faster-whisper：

```bash
python3 -m pip install -U faster-whisper
```

5. 运行转写脚本，中文录音优先指定 `--language zh`；Apple Silicon 上显式指定 `--engine mlx`，不确定语言时省略语言参数：

```bash
/tmp/local-audio-transcriber-mlx/bin/python {skill_dir}/scripts/transcribe.py "input.m4a" --engine mlx --language zh --formats txt,md,json --print-text
```
...
/tmp/local-audio-transcriber-mlx/bin/python {skill_dir}/scripts/transcribe.py "recording.m4a" --engine mlx --language zh --formats txt,md,json,srt,vtt --print-text
...
# 非 Apple Silicon / CUDA / 用户明确同意 CPU fallback：使用 faster-whisper
...
- Apple Silicon 禁止自动 CPU fallback：如果 MLX 缺失、Metal/GPU 不可用或 MLX 转写异常变慢，先修复 MLX、换 MLX 模型、关闭初始提示词或调整音频切分；只有用户明确确认后，才能改用 `faster-whisper` CPU
...
- 如果 MLX `whisper-large-v3-turbo-q4` 在某段异常慢，优先尝试：去掉过长 `--initial-prompt`、换 `mlx-community/whisper-small-mlx` 或先用 `ffmpeg` 切成 10-15 分钟片段后继续用 `--engine mlx`；不要擅自切 CPU
...
- `mlx` 默认设备不是 `Device(gpu, 0)`：说明 MLX/Metal 没走通；Apple Silicon 上先检查系统环境或重装 MLX，不要自动降级到 faster-whisper CPU
- 音频无法解码：建议安装或更新 `ffmpeg`，或让用户换成 `.m4a/.mp3/.wav`
- 转写很慢：Apple Silicon 上仍保持 `--engine mlx`，改用 MLX 小模型、去掉过长提示词或切分音频；非 Apple Silicon 才考虑 CPU 小模型
